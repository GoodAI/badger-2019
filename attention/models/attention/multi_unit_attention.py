...
from attention.agents.agent import RolloutAware, T_INIT_ROLLOUT_PARAMS
...
class RolloutAwareAndStore(RolloutAware[T_INIT_ROLLOUT_PARAMS], Generic[T_INIT_ROLLOUT_PARAMS]):
    n_experts: int
    batch_size: int
...
class AttentionUnit(Generic[T_PARAM, T_RESULT, T_INIT_ROLLOUT_PARAMS], RolloutAwareAndStore[T_INIT_ROLLOUT_PARAMS], ABC):
...
class MultiUnitAttention(RolloutAwareAndStore[T_INIT_ROLLOUT_PARAMS], Generic[T_PARAM, T_INIT_ROLLOUT_PARAMS]):
    _queries_per_unit: List[int] = None

    def __init__(self, units: List[AttentionUnit[T_PARAM, Tensor, T_INIT_ROLLOUT_PARAMS]], result_callback: Callable[[List[Tensor]], None],
    ...
    def init_rollout(self, batch_size: int, n_experts: int, params: T_INIT_ROLLOUT_PARAMS):
        for unit in self.units:
            unit.init_rollout(batch_size, n_experts, params)
