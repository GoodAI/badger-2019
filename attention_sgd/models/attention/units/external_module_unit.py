...
from attention_sgd.models.attention.multi_unit_attention import AttentionUnit, AttentionMessages
from attention_sgd.models.with_module import WithModule
from attention_sgd.utils.torch_utils import expand_to_batch_size

T_P = TypeVar('T_P')
T_R = TypeVar('T_R')


class ExternalModuleUnit(AttentionUnit[T_P, Tensor], WithModule, Generic[T_P]):
    _m: nn.Module
...
class FixedKeyUnit(ExternalModuleUnit[T_P], Generic[T_P]):
...
class FixedQueryUnit(ExternalModuleUnit[T_P], Generic[T_P]):
...
class ExternalModuleWriteUnit(ExternalModuleUnit[T_P], Generic[T_P]):
...
class ExternalModuleReadUnit(ExternalModuleUnit[T_P], Generic[T_P]):
...
class FilterAttentionUnit(AttentionUnit[T_P, T_R]):
    def __init__(self, unit: AttentionUnit[T_P, T_R]):
        self.unit = unit
...
class OutputOnlyFilterAttentionUnit(FilterAttentionUnit[T_P, T_R]):
